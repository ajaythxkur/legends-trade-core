...
export default function Body() {
    ...
    //                 backendApi.getUserData(String(account.address)),
    //                 backendApi.getUserPremarketTokens(String(account.address), 10, offset, tokenStatus)
    //             ]);

    //             setUserData(userRes.data);
    //             setTokens(tokensRes.data.tokens);
    //             setTotal(tokensRes.data.total);
    //         } catch (err) {
        ...
        <>
            <div className="md:p-4">
                <div className="flex items-center gap-4">
                    <Image src="/media/Logo.svg" alt="user-img" height={40} width={40} className="rounded-full" />
                    <H6 className="text-primary-text-color font-bold flex gap-4 items-center">
                        {shortAddress(String(account?.address))}
                        <CopyAddress address={account.address.toString()} />
                    </H6>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 justify-between mt-6 lg:mt-8 xl:mt-10 2xl:mt-12">
                    ...
                    <div className="bg-card-bg p-4 rounded-lg flex-1">
                        ...
                        <H5 className="text-text-emphasis mt-[10px]">$ 20.00</H5>
